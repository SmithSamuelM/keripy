...
class Reger(dbing.LMDBer):
    ...
    def logCred(self, creder, sadsigers=None, sadcigars=None):
        ...
        if sadcigars:
            for (pather, cigar) in sadcigars:
                keys = (creder.said, pather.qb64)
                self.spcgs.put(keys=keys, vals=[(cigar.verfer, cigar)])
        if sadsigers:  # want sn in numerical order so use hex
            for (pather, prefixer, seqner, saider, sigers) in sadsigers:
                quinkeys = (creder.said, pather.qb64, prefixer.qb64, f"{seqner.sn:032x}", saider.qb64)
                for siger in sigers:
    ...
    def cloneCred(self, said, root=None):
        ...
        for keys, cigar in self.spcgs.getItemIter(keys=(creder.said, "")):
        ...
        sigers = []
        old = None  # empty keys
        for keys, siger in self.spsgs.getItemIter(keys=(creder.said, "")):
            quad = keys[1:]
            if quad != old:  # new tsg
